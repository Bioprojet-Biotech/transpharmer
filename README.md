...
This creates `.venv` and installs the locked dependencies (Python 3.9, PyTorch 1.13.1 CUDA 11.7 wheels, RDKit, etc.). Run commands with `uv run`, or `source .venv/bin/activate`.

For GuacaMol benchmarking (pulls TensorFlow 2.11.0):

```bash
uv sync --extra benchmark
```

The original conda/mamba recipe is kept in `other_requirements.yml` as a reference. TransPharmer was tested with:
...
```
uv run python generate.py --config configs/generate_pc.yaml
```
...
```shell
uv run python generate.py --config configs/generate_reproduce_plk1.yaml
```
